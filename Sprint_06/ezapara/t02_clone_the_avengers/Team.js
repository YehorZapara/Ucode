exports.Team = class {
  constructor(item, avengers) {
    this.item = item;
    this.avengers = avengers;
  }

  clone(clonedTeam) {
    return clonedTeam = { ...this };
  }

  battle(damage) {
    this.avengers.map((item) => {
      item.damage(damage);
    });
  ...
  calculateLosses(clonedTeam) {
    if (this.avengers.length == clonedTeam.avengers.length) {
      ...
      let losts = clonedTeam.avengers.length - this.avengers.length;
      console.log(`In this battle we lost ${losts} Avengers`);
    }
  }
};
